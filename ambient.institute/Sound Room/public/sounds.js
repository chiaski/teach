...
//var socket = io('https://ambient.institute');
...
$("#sounds").on("click", ".sound span[text]", function(){
  ...
  toggleSound(sound, i, sounds[i].active);
  socket.emit('toggleSound', sound, i, sounds[i].active);
  console.log("socket emitttt");
});

socket.on('toggleSound', function(sound, i, state) { 
  toggleSound(sound, i, state);
  console.log("testing" + sound + state);
});

function toggleSound(sound, i, state){
  sounds[i].active = state;
  ...
  localStorage.setItem( (sound + ".active"), state); 
}
...
$("#sounds").on("click", ".sound button[pitch]", function(){
  ...
  sounds[i].pitchControl = sounds[i].pitchControl ? false : true;
  
  togglePitch(sound, i, sounds[i].pitchControl);
  socket.emit('togglePitch', sound, i, sounds[i].pitchControl);
  
});


socket.on('togglePitch', function(sound, i, state) { togglePitch(sound, i, state) });

function togglePitch(sound, i, state){
  
  sounds[i].pitchControl = state;
  
  if(state){
     $(".sound[name='" + sound + "'] audio")[0].preservesPitch = false;
     $(".sound[name='" + sound + "'] button[pitch]").attr("off", "");
  } else{
     $(".sound[name='" + sound + "'] audio")[0].preservesPitch = true;
      $(".sound[name='" + sound + "'] button[pitch]").removeAttr("off");
  }
...
$("#sounds").on("click", ".sound button[playback]", function(){
  ...
  let sound = $(this).parent().parent().attr("name");
  let i = $(this).parent().parent().attr("i");  
  let rate = $(".sound[name='" + sound + "'] audio")[0].playbackRate;
  
  if( $(this).attr("control") == "slow" ){
    if(sounds[i].rate <= 0.12) return;
    sounds[i].rate -= 0.08;
  } else{
    sounds[i].rate += 0.08;
  }
  
  adjustPlayback(sound, i, sounds[i].rate);  
  socket.emit('adjustPlayback', sound, i, sounds[i].rate);
});

socket.on('adjustPlayback', function(sound, i, state) { adjustPlayback(sound, i, state) });


function adjustPlayback(sound, i, rate){

  $(".sound[name='" + sound + "'] span[text]").css("opacity", rate / 2);    
  $(".sound[name='" + sound + "'] audio")[0].playbackRate = rate;
...
$("#sounds").on("click", ".sound button[volume]", function(){
  ...
  if( $(this).attr("control") == "soft" ){
    if(sounds[i].volume <= 0.12) return;
    sounds[i].volume -= 0.08;
  } else{
    sounds[i].volume += 0.08;
  }

  
  adjustVolume(sound, i, rate);
  socket.emit('adjustVolume', sound, i, sounds[i].volume);
});

socket.on('adjustVolume', function(sound, i, rate) { adjustVolume(sound, i, rate) });

function adjustVolume(sound, i, rate){
  $(".sound[name='" + sound + "'] audio")[0].volume = sounds[i].volume;
